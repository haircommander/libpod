...
func (r *LocalRuntime) Exec(ctx context.Context, c *cliconfig.ExecValues, cmd []string) (int, error) {
	var ctr *Container
	var err error
	...
	if err != nil {
		return 125, errors.Wrapf(err, "unable to exec into %s", c.InputArgs[0])
	}

	if c.PreserveFDs > 0 {
		entries, err := ioutil.ReadDir("/proc/self/fd")
		if err != nil {
			return 125, errors.Wrapf(err, "unable to read /proc/self/fd")
		}
		m := make(map[int]bool)
		for _, e := range entries {
			i, err := strconv.Atoi(e.Name())
			if err != nil {
				if err != nil {
					return 125, errors.Wrapf(err, "cannot parse %s in /proc/self/fd", e.Name())
				}
			}
			m[i] = true
		}
		for i := 3; i < 3+c.PreserveFDs; i++ {
			if _, found := m[i]; !found {
				return 125, errors.New("invalid --preserve-fds=N specified. Not enough FDs available")
			}
		}
	}
	...
	if err := parse.ReadKVStrings(env, []string{}, c.Env); err != nil {
		return 125, errors.Wrapf(err, "unable to process environment variables")
	}
	envs := []string{}
	for k, v := range env {
		envs = append(envs, fmt.Sprintf("%s=%s", k, v))
	...
	return ExecAttachCtr(ctx, ctr.Container, c.Tty, c.Privileged, envs, cmd, c.User, c.Workdir, streams, c.PreserveFDs, c.DetachKeys)
}
