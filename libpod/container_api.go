...
func (c *Container) Exec(tty, privileged bool, env, cmd []string, user, workDir string, streams *AttachStreams, preserveFDs int, resize chan remotecommand.TerminalSize, detachKeys string) (int, error) {
	...
	if !c.batched {
		...
		if err := c.syncContainer(); err != nil {
			return defaultExecExitCode, err
		}
	}
	...
	if conState != ContainerStateRunning {
		return defaultExecExitCode, errors.Errorf("cannot exec into container that is not running")
	}
	...
	pid, attachChan, err := c.runtime.ociRuntime.execContainer(c, cmd, capList, env, tty, workDir, user, sessionID, streams, preserveFDs, resize, detachKeys)
	if err != nil {
